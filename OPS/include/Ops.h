...
#include <pcl/kdtree/kdtree_flann.h>          //Nearest neighbor
...
using OPSPlane = std::pair<std::vector<pcl::PointXYZ>, std::pair<Eigen::Vector3f, Eigen::Vector3f>>; //(inliers, (normal, centroid))

namespace defaultParams
{
    const double          samplingRate         = 0.1;
    const int             KNN                  = 30;
    const double          threshDistToOPSPlane = 0.05; // meters
    const float           pRansacAdaptive      = 0.99;
    const int             minInliersOnOPSPlane = 40;
...
std::vector<OPSPlane> process(const pcl::PointCloudXYZ::Ptr cloud,
                              ...
                              const double                  alphaS             = defaultParams::samplingRate,
                              const int                     K                  = defaultParams::KNN,
                              const double                  threshDistOPSPlane = defaultParams::threshDistToOPSPlane,
                              const int                     threshInliers      = defaultParams::minInliersOnOPSPlane,
                              const float                   threshAngle        = defaultParams::threshAngleToAxis,
                              const float                   p                  = defaultParams::pRansacAdaptive);
...
Eigen::Vector3f computeSVDNormal(const std::vector<int>& nnIdx, const int piIdx, const pcl::PointCloudXYZ::Ptr cloud, const double sigma = 1);

void visualizeOPSPlanes(const std::vector<OPSPlane>& OPSPlanes, const pcl::PointCloudXYZ::Ptr remainingCloud);
