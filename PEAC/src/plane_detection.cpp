...
bool PlaneDetection::setColorImage(cv::Mat& image)
{
    color_img_ = image;
...
bool PlaneDetection::setDepthImage(cv::Mat& image)
{
    cv::Mat depth_img = image;
...
void PlaneDetection::prepareForMRF()
{
    ...
    for (int row = 0; row < kDepthHeight; ++row)
    {
        for (int col = 0; col < kDepthWidth; ++col)
        {
            ...
            if ((row - 1 >= 0 && mat_label.at<int>(row - 1, col) != label) || (row + 1 < kDepthHeight && mat_label.at<int>(row + 1, col) != label) ||
                (col - 1 >= 0 && mat_label.at<int>(row, col - 1) != label) || (col + 1 < kDepthWidth && mat_label.at<int>(row, col + 1) != label))
            {
...
void PlaneDetection::writePlaneDataFile(string filename, bool run_mrf /* = false */)
{
    ...
    for (int pidx = 0; pidx < plane_num_; ++pidx)
    {
        ...
        if (run_mrf)
        {
            out << opt_sum_stats_[pidx].sx << std::setprecision(8) << " " << opt_sum_stats_[pidx].sy << std::setprecision(8) << " " << opt_sum_stats_[pidx].sz
                << std::setprecision(8) << " " << opt_sum_stats_[pidx].sxx << std::setprecision(8) << " " << opt_sum_stats_[pidx].syy << std::setprecision(8)
                << " " << opt_sum_stats_[pidx].szz << std::setprecision(8) << " " << opt_sum_stats_[pidx].sxy << std::setprecision(8) << " "
                << opt_sum_stats_[pidx].syz << std::setprecision(8) << " " << opt_sum_stats_[pidx].sxz << std::setprecision(8) << endl;
        }
        else
        {
            out << sum_stats_[pidx].sx << std::setprecision(8) << " " << sum_stats_[pidx].sy << std::setprecision(8) << " " << sum_stats_[pidx].sz
                << std::setprecision(8) << " " << sum_stats_[pidx].sxx << std::setprecision(8) << " " << sum_stats_[pidx].syy << std::setprecision(8) << " "
                << sum_stats_[pidx].szz << std::setprecision(8) << " " << sum_stats_[pidx].sxy << std::setprecision(8) << " " << sum_stats_[pidx].syz
                << std::setprecision(8) << " " << sum_stats_[pidx].sxz << std::setprecision(8) << endl;
        }
        ...
        // cout << stat.sx << " " << stat.sy << " " << stat.sz << " " << stat.sxx << " "<< stat.syy << " "<< stat.szz << " "<< stat.sxy << " "<< stat.syz << "
        // "<< stat.sxz << endl;
    }
    out.close();
}
