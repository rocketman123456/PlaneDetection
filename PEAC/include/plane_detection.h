...
struct SumStats
{
    ...
    SumStats() { sx = sy = sz = sxx = syy = szz = sxy = syz = sxz = 0; }
};
...
public:
    ...
    bool setColorImage(cv::Mat& image);
    bool setDepthImage(cv::Mat& image);
...
private:
    ...
    {
        return int(0.299 * color_img_.at<cv::Vec3b>(x, y)[2] + 0.587 * color_img_.at<cv::Vec3b>(x, y)[1] + 0.114 * color_img_.at<cv::Vec3b>(x, y)[0] + 0.5);
    }
